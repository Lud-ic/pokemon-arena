...
import { searchByName, selectType, percentageCalc,selectRarity, sortAZ, selectWeaknesses, selectResistant } from './data.js'
...
filterResistant.addEventListener("change", (e) => {
  ...
  const resistantPercentage = `${percentageCalc(pokemons.length,arrResistant.length)}% POKÉMONS FROM THIS CATEGORY`
...
filterWeaknesses.addEventListener("change", (e) => {
  ...
  const WeaknessesPercentage = `${percentageCalc(pokemons.length,arrWeaknesses.length)}% POKÉMONS FROM THIS CATEGORY`
...
filterRarity.addEventListener("change", (e) => {
  ...
  const typePercentage = `${percentageCalc(pokemons.length,arrRarity.length)}% POKÉMONS FROM THIS CATEGORY`
  showPercentage(typePercentage);
})
